...
function event_list_cal_columns_data( $column, $post_id ) {
  switch ( $column ) {
	...
	case "event-repeat":
		...
		if($event_repeat == 1) {
			$event_repeat = __('Týdně', 'event-list-calendar');
		} elseif($event_repeat == 2) {
			$event_repeat = __('Měsíčně', 'event-list-calendar');
		} elseif($event_repeat == 3) {
			$event_repeat = __('Ročně', 'event-list-calendar');
		} else {
			$event_repeat = __('Jednorázová akce', 'event-list-calendar');
		}
		echo $event_repeat;
		break;
	case "event-days":
		...
		if(empty($event_days)) {
			$event_days = $event_days.__('1 den', 'event-list-calendar');
		} elseif($event_days == 1) {
			$event_days = $event_days.__(' den', 'event-list-calendar');
		} else {
			$event_days = $event_days.__(' dny', 'event-list-calendar');
		}
		echo $event_days;
		break;
  }
...
function event_list_cal( $atts ) {
    ...
    $events[] = "<a href=\"".get_permalink($loop->ID)."\" class=".$kategorie.">".get_the_title()."</a>==".$event_date."==".$event_time."==<a href=\"".get_permalink($loop->ID)."\">&nbsp;</a>".get_the_excerpt()."==".$event_days."==".$event_repeat_schedule."==".$event_end;
	endwhile;
...
<div id=\"event-list-cal-container\">
	...
	if(get_settings('start_of_week') != 1) {
		$cal_output .= "
					<th>".__( 'Neděle', 'event-list-calendar' )."</th>";
	}
	$cal_output .= "
					<th>".__( 'Pondělí', 'event-list-calendar' )."</th>
					<th>".__( 'Úterý', 'event-list-calendar' )."</th>
					<th>".__( 'Středa', 'event-list-calendar' )."</th>
					<th>".__( 'Čtvrtek', 'event-list-calendar' )."</th>
					<th>".__( 'Pátek', 'event-list-calendar' )."</th>
					<th>".__( 'Sobota', 'event-list-calendar' )."</th>";
	if(get_settings('start_of_week') == 1) {
		$cal_output .= "
					<th>".__( 'Neděle', 'event-list-calendar' )."</th>";
	}
	...
	for($j = 1; $j <= $num_days; $j++) {
		...
		foreach($events as $event) {
			...
			if($event_year == $year && $event_month == $month && $event_day == $j) {
				...
				if(!empty($event_time)) {
					$cal_output .= "
								<p class=\"event-list-cal-time\">Čas události: ".$event_time."</p>";
				}
				$cal_output .= "
								".$event_excerpt."
							</div>
...
";
    ...
    $list_output .= "\t<li class=\"upcoming-event-".$j."\"><div class=\"event-list-cal-date\"><span class=\"event-list-cal-day-month\">".date('j. n.', $date)."</span><span class=\"event-list-cal-year\">".date('Y', $date)."</span></div><span class=\"event-list-cal-title\">".$event_link."</span></li>
";
		$j++;
	}
	if(empty($events)) {
		$list_output .= "\t<li class=\"upcoming-event-no-events\">".__('Nejsou žádné blížící se události.', 'event-list-calendar')."</li>
";
	}
...
function event_list_mini_cal_callback() {
	...
	if(get_settings('start_of_week') != 1) {
		$cal_output .= "
					<th>".__( 'Ne', 'event-list-calendar' )."</th>";
	}
	$cal_output .= "
					<th>".__( 'Po', 'event-list-calendar' )."</th>
					<th>".__( 'Út', 'event-list-calendar' )."</th>
					<th>".__( 'St', 'event-list-calendar' )."</th>
					<th>".__( 'Čt', 'event-list-calendar' )."</th>
					<th>".__( 'Pá', 'event-list-calendar' )."</th>
					<th>".__( 'So', 'event-list-calendar' )."</th>";
	if(get_settings('start_of_week') == 1) {
		$cal_output .= "
					<th>".__( 'Ne', 'event-list-calendar' )."</th>";
	}
