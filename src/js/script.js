...
const swpCal = {
  ...
  getMonths (relMonth) {
    ...
    this.numOfDays = Math.floor((this.firstDayOfNextMonth - this.firstDayOfMonth) / (1000 * 60 * 60 * 24));
    this.daysArr = [];
   ...
   * @param {DOMElement} elm Elm v kalendáři se dnem ke kterému budu novou událost připojovat
   * @todo opravit bug 2.11. --> po dvojkliku se zdvojí "Opakující se vícedenní akce"
   * @return Vytvoří HTML pro event a připojí ho k elm
   */
    ...
    while (this.daysArr.length > 0) {
      for (let i = 1; i < 8; i++) {
        if (i >= firstDay && !firstWeekConst) { // Případ, kdy měsíc začíná o víkendu
          const daysArrItem = this.daysArr.shift();
          const dayElm = this.createElm(daysArrItem);
  ...
  addEventOneDay (event) {
    ...
    if (parseInt(date[1]) === thisMonth && parseInt(date[0]) === thisYear) {
      const elm = document.querySelector(`#swp-cal-mini-main #calendar .days #day-${date[2]}`);
  ...
  addEventMultipleDays (event, thisMonthStr) {
    ...
    // case 2 akce z minulého měsíce: začíná minulý měsíc a zároveň jeho datumStartu
    // + početDní je větší, než počet dní toho měsíce
  ...
  addEventRepeated (event) {
    ...
    default:
      console.error(`Error. Something went wrong with event "${event.title}". 
        Check the event from your wordpress admin interface.`);
      break;
    }

    /**
     * Jako jediná se může zobrazovat i před prvním datem akce (svátky, výročí,
     * navíc je to tak daleko že je to jedno) pokud je forever
     */
    ...
    /**
     * Adds event that is repeated monthly
     */
    ...
    /**
     * Adds event that is repeated weekly
     */
    function addRepeatedWeekly () {
      ...
      while (currentEventDayOfMonth <= swpCal.lastDayOfMonth.getDate()) {
        ...
        const elm = document.querySelector(`#swp-cal-mini-main #calendar .days #day-${str}`);
  ...
  renderList (events, size) {
          ...
          for (let n = 0; n < size; n++) {
            ...
            if (dayOfWeeklyEvent >= this.today) {
              ...
              /**
                 ...
                 * tady je potřeba
                 */
              const diff = this.today - dayOfWeeklyEvent;
    ...
    const isContainer = !!document.querySelector('ul.swp-list');
    ...
    for (let y = 0; y < size; y++) {
      const eventElm = this.createListItem(upcomingEvents[y], y);
  ...
  createListItem (event, iter) {
    const date = event.eventDate.split('-');
    const year = date[0];
    const yearFits = this.today.getFullYear() === parseInt(year);
